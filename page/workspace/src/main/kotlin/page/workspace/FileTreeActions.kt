...
object FileTreeActions {
    ...
    fun renameDirectoryByCopy(source: Path, target: Path) {
    ...
    fun copyTree(source: Path, target: Path) = copyTreeRecursive(source, target)

    fun deleteTreeWithRetry(path: Path, attempts: Int = 5, sleepMs: Long = 200L): Boolean {
        for (i in 1..attempts) {
            val ok = runCatching {
