...
object TreeSitterJavaLexer : SyntaxLexer {
    ...
    override fun tokenize(text: String): List<Token> {
        ...
        walk(cursor, byteToChar, text.length, out)
        return out
    }

    private fun walk(c: TSTreeCursor, byteToChar: IntArray, charLen: Int, out: MutableList<Token>) {
        ...
        if (kind != null) {
            ...
            if (end > start) out += Token(kind, start until end)
            return
        }
        if (c.gotoFirstChild()) {
            do {
                walk(c, byteToChar, charLen, out)
